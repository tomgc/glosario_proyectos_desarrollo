...
*Contexto:* En tus proyectos, la auditoría de cifras sigue un patrón de tres scripts (helpers + orquestador de familias + spot-check): cada cifra se calcula por dos caminos independientes y se compara con una tolerancia definida; una familia que falla no aborta la validación de las demás.
...
*Contexto:* En tu flujo de trabajo, el backlog es append-only: las entradas previas nunca se editan ni renumeran, y un error se corrige agregando una entrada nueva, no modificando la anterior.
...
*Contexto:* En tu flujo, los commits van en español, con mensajes descriptivos que explican el "por qué" del cambio, no solo el "qué"; las operaciones de Git como `git push` y `gh pr create` se ejecutan siempre de forma individual, nunca encadenadas con `&&`.
...
*Contexto:* Trabajas con repos privados en GitHub bajo el plan Free, lo que implica que ramas privadas no tienen branch protection nativa; lo compensas con disciplina de Pull Request documentada, Secret Scanning y Dependabot habilitados, y flujo de squash merge con eliminación automática de la rama.
...
*Contexto:* En tus proyectos, la gobernanza de datos exige que datos personales (RUT, nombres, resultados individuales) nunca entren al repositorio Git: la separación entre raíz de código (Git) y raíz de datos (OneDrive institucional) es física, no solo un `.gitignore`.
...
*Contexto:* En tu flujo de trabajo, distingues explícitamente el rol de dos LLM: Claude chat como colaborador estratégico (planificación, decisiones de diseño, redacción de documentos) y Claude Code como entorno de ejecución (operaciones de archivo, comandos Git).
...
*Contexto:* En tus auditorías con datos y llaves sensibles de por medio, el panel adversarial es un paso obligatorio post-cómputo: re-deriva cifras clave desde el parquet fuente con lógica propia, valida invariantes de reconciliación (matrícula total por año, no doble-conteo por serie, retención de establecimiento ≤ retención de sistema) y para de inmediato si alguno falla, reportando el hallazgo con evidencia.
...
*Contexto:* En tu backlog, "reuso y patrón de familia" agrupa casos como funciones de paleta de colores, rutas auxiliares o utilidades compartidas (d3, pako) que se busca centralizar entre los proyectos hermanos, distinguiendo lo que es genuinamente byte-idéntico (seguro de extraer) de lo que tiene firmas incompatibles entre proyectos (requiere resolución explícita antes de extraerse).
